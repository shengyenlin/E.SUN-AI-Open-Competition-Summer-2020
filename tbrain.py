...
def model2(input):
  ...
  def loca_test(x):
    res = pd.DataFrame.bool(x <=15) and first
    res2 = pd.DataFrame.bool(x>= (len(input)-15)) and last
    res3 = res or res2
    return res3
  name_id['journalist'] = 0
  for i in range(len(name_id)):
    if (loca_test(name_id.iloc[i,3])) == True:
      name_id.iloc[i,6] = 1
    else:
      name_id.iloc[i,6] = 0
  name_id = name_id[name_id['journalist']==0]
  # 只猜三字
